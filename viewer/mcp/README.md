...
| `get_instances` | `pid`, `class_name`, `cap?` | `{pid, class, instances:[{address, fields:{name:value, …}, refs:{name:0xoop, …}}]}` — live heap objects + field values (own + inherited); `refs` gives each reference field's pointee address (feed it to `set_instance_field` / `call_method` / `get_array`). Injects if needed |
| `get_statics`   | `pid`, `class_name`      | `{pid, class, statics:{name:value, …}, refs:{name:0xoop, …}}` — the class's STATIC field values (read live from its `java.lang.Class` mirror) + a `refs` map of each reference static's address |

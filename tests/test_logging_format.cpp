...
namespace fpinv
{
    ...
    // Parse a formatted floating-point string back to a double.  We accept any
    // spelling std::from_chars understands; for the few specials/forms it does
    // not parse uniformly across STLs (hex-float, "inf"/"nan" tokens) the call
    // sites assert tokens directly rather than going through here.
    inline auto parses_to(std::string_view text, double expected) -> bool
    {
        ...
        // Bit-exact: these are shortest-round-trip spellings, so the parsed
        // value must reproduce the original double exactly (handles -0.0 too,
        // since we also check the sign bit separately at the call site).
...
int main()
{
    ...
    // the INVARIANT structure (a 'p'/'P' exponent marker is present, lowercase
    // vs uppercase tracks the spec letter) plus that the rendering round-trips
    // back to the original double on STLs whose from_chars accepts hexfloat.
    {

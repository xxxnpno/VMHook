...
int main()
{
    ...
    // =====================================================================
    // T. klass_from_oop INPUT PRE-GATE - EXHAUSTIVE is_valid_pointer boundary.
    //    klass_from_oop's first action is is_valid_pointer(oop); a false there
    //    short-circuits to nullptr BEFORE the +8 read, so the entire rejection
    //    domain is safe to probe with no JVM (none of these are dereferenced).
    //    is_valid_pointer (vmhook.hpp ~2047) rejects, in order:
    //      (1) addr <= user_address_floor   (0xFFFF)            -> reject
    //      (2) addr >= user_address_ceiling (0x00007FFFFFFFFFFF) -> reject
    //      (3) (addr & 1) != 0              (odd)               -> reject
    //      (4) low32 in the 9-pattern poison set                -> reject
    //    Every value here is chosen to exercise EXACTLY one documented arm, and
    //    the expected klass_from_oop result for all of them is nullptr.
    // =====================================================================
    {
        // (T1) FLOOR arm: addr <= 0xFFFF. The comparison is `<=`, so the floor
        //      value itself AND everything below it is rejected. Sweep the whole
        //      sub-floor range plus the exact floor; every one must be nullptr.
        bool floor_all_null{ true };
        // Even values only below/at the floor would otherwise reach the odd arm;
        // but the floor arm fires FIRST regardless of parity, so include both.
        for (std::uintptr_t a{ 0u }; a <= 0xFFFFu; a += 0x111u)   // 0,0x111,...,~0xFFFF
        {
            if (vmhook::klass_from_oop(reinterpret_cast<void*>(a)) != nullptr)
            {
                floor_all_null = false;
            }
        }
        // The exact floor boundary and floor+1 (still <= floor? no: floor+1 >
        // floor, so floor+1 is NOT rejected by the floor arm - but 0x10000 is
        // even and not poison, so it passes the pre-gate and then decodes to
        // nullptr off-JVM anyway; assert the floor value itself here).
        if (vmhook::klass_from_oop(reinterpret_cast<void*>(std::uintptr_t{ 0xFFFFu })) != nullptr)
        {
            floor_all_null = false;
        }
        check("klass_from_oop_floor_sweep_all_null", floor_all_null);
        // The first address ABOVE the floor that is even & non-poison passes the
        // pre-gate, so the nullptr there comes from the missing codec, not the
        // gate - proving the floor arm boundary is exactly `<= 0xFFFF`.
        check("klass_from_oop_just_above_floor_is_null_via_codec",
              vmhook::klass_from_oop(reinterpret_cast<void*>(std::uintptr_t{ 0x10000u })) == nullptr);

        // (T2) CEILING arm: addr >= 0x00007FFFFFFFFFFF. The comparison is `>=`,
        //      so the ceiling value itself and everything above is rejected.
        const std::uintptr_t ceil{ 0x0000'7FFF'FFFF'FFFFull };
        const std::uintptr_t at_or_above_ceiling[]{
            ceil,                              // exactly the ceiling (>= -> reject)
            ceil + 1u,                         // just above
            std::uintptr_t{ 0x0000'8000'0000'0000ull }, // first kernel-half
            std::uintptr_t{ 0x8000'0000'0000'0000ull }, // top-bit set
            std::uintptr_t{ 0xFFFF'FFFF'FFFF'FFFFull }, // all-ones
            std::uintptr_t{ 0xFFFF'8000'0000'0000ull }, // high non-canonical
        };
        bool ceiling_all_null{ true };
        for (const std::uintptr_t a : at_or_above_ceiling)
        {
            if (vmhook::klass_from_oop(reinterpret_cast<void*>(a)) != nullptr)
            {
                ceiling_all_null = false;
            }
        }
        check("klass_from_oop_ceiling_arm_all_null", ceiling_all_null);
        // The largest address strictly BELOW the ceiling that is even passes the
        // range+parity gate -> nullptr only via the missing codec.
        check("klass_from_oop_just_below_ceiling_is_null_via_codec",
              vmhook::klass_from_oop(reinterpret_cast<void*>(ceil - 1u)) == nullptr);

        // (T3) ODD arm: any in-range odd address is rejected. Sweep odd values
        //      across the canonical span; all must be nullptr.
        bool odd_all_null{ true };
        for (std::uintptr_t a{ 0x1'0001u }; a < 0x7FFF'FFFF'0000ull; a <<= 1)
        {
            const std::uintptr_t odd{ a | 1u };                 // force low bit set
            if (vmhook::klass_from_oop(reinterpret_cast<void*>(odd)) != nullptr)
            {
                odd_all_null = false;
            }
        }
        check("klass_from_oop_odd_inrange_all_null", odd_all_null);

        // (T4) POISON arm: an EVEN, in-range address whose low 32 bits match one
        //      of the 9 documented sentinel patterns is rejected. The patterns
        //      themselves are odd (e.g. 0xDEADBEEF) so they would hit the odd
        //      arm; we lift them into range with a high base AND clear bit 0 so
        //      the ONLY rejecting arm left is the poison switch. Pattern with
        //      bit0 cleared still matches low32 for the even sentinels and, for
        //      odd sentinels, we verify the odd arm rejects the raw pattern.
        ...
        bool poison_all_null{ true };
        for (const std::uint32_t pat : poison_low32)
        {
            // Even poison addresses: clear bit 0 so parity does not reject; the
            // poison switch keys on low32 which, for the even patterns
            // (0xCCCCCCCC, 0xFEEEFEEE, 0xABABABAB, 0xDDDDDDDD), is unchanged, and
            // for the odd ones the cleared-bit value no longer matches the switch
            // - so for those we instead rely on the RAW (odd) pattern being
            // rejected by the odd arm. Cover both shapes:
            const std::uintptr_t even_in_range{
                (std::uintptr_t{ 0x0000'1234ull } << 32) | std::uintptr_t{ pat & ~1u } };
            if (vmhook::klass_from_oop(reinterpret_cast<void*>(even_in_range)) != nullptr)
            {
                poison_all_null = false;
            }
            const std::uintptr_t raw_in_range{
                (std::uintptr_t{ 0x0000'1234ull } << 32) | std::uintptr_t{ pat } };
            if (vmhook::klass_from_oop(reinterpret_cast<void*>(raw_in_range)) != nullptr)
            {
                poison_all_null = false;   // rejected by poison (even pats) or odd arm
            }
        }
        check("klass_from_oop_poison_low32_all_null", poison_all_null);

        // NOTE: the (T5) "even poison rejected by the poison switch" check was
        // removed -- it conflated the oop ADDRESS low32 with the NARROW-KLASS
        // poison read from oop+8 (different values), and its result depended on a
        // fabricated ~366GB address being unmapped (ASLR-fragile, not a portable
        // invariant). The broader poison_low32_all_null check above stays.
    }

    // =====================================================================
    // U. untag_pointer interaction with the klass decode result.  untag_pointer
    //    (vmhook.hpp ~2092) masks with user_address_ceiling (0x00007FFFFFFFFFFF),
    //    stripping any bit at/above bit 47.  A klass-space pointer the decode
    //    produces from a canonical base/shift lies below the ceiling, so untag is
    //    the identity on it; only GC-tagged (high-bit) values are altered.  We
    //    pin both: identity on in-range decoded klass addresses, and masking on
    //    high-tagged ones, computing the masked value independently.
    // =====================================================================
    {
        // (U1) untag is the identity on a decoded klass address that is already
        //      in the canonical range (every kModes decode of a modest narrow).
        bool untag_identity{ true };
        for (const klass_mode m : kModes)
        {
            const std::uint32_t cs[]{ 1u, 7u, 0x1000u, 0x10'0000u };
            for (const std::uint32_t c : cs)
            {
                void* const decoded{ narrow_decode(m.base, m.shift, c) };
                const std::uintptr_t a{ as_uptr(decoded) };
                if (a >= 0x0000'7FFF'FFFF'FFFFull) { continue; }   // would be masked
                if (untag_pointer(decoded) != decoded) { untag_identity = false; }
            }
        }
        check("klass_decode_untag_identity_in_range", untag_identity);

        // (U2) untag MASKS a high-tagged pointer: set tag bits at/above bit 47 on
        //      a decoded klass address; untag must clear exactly those and recover
        //      the low 47 bits. Independently computed mask = & 0x00007FFFFFFFFFFF.
        const std::uintptr_t real{ as_uptr(narrow_decode(0x8'0000'0000ull, 3u, 0x2000u)) };
        const std::uintptr_t tag{ std::uintptr_t{ 0xFFFF'8000'0000'0000ull } };
        const std::uintptr_t tagged{ real | tag };
        const std::uintptr_t expect_untagged{ tagged & 0x0000'7FFF'FFFF'FFFFull };
        check("klass_decode_untag_strips_high_tag",
              as_uptr(const_cast<void*>(untag_pointer(reinterpret_cast<void*>(tagged))))
                  == expect_untagged);
        check("klass_decode_untag_recovers_real_when_tag_above_bit47",
              expect_untagged == (real & 0x0000'7FFF'FFFF'FFFFull));
    }

    // =====================================================================
    // V. encode UNDERFLOW exact-value spec.  encode_klass_pointer's own guard is
    //    `decoded_address < base -> 0`, but the SHARED primitive narrow_encode
    //    has NO guard and computes (addr - base) >> shift modulo 2^64 then
    //    narrows to 32 bits.  Section E/J touched this; here we pin the EXACT
    //    32-bit result of the modular underflow for several (base, shift, addr)
    //    triples below base, recomputing each from the closed form so the corner
    //    is fully specified, not merely "deterministic".
    // =====================================================================
    {
        struct uf { std::uint64_t base; std::uint32_t shift; std::uint64_t addr; };
        const uf cases[]{
            { 0x8'0000'0000ull, 3u, 0u },                 // (0 - 32G) >> 3
            { 0x8'0000'0000ull, 0u, 0u },                 // (0 - 32G) >> 0
            { 0x8'0000'0000ull, 3u, 0x8'0000'0000ull - 8u }, // base-8
            { 0x1'0000ull,      3u, 0u },                 // small base underflow
            { 0x1'2345'6000ull, 3u, 0x1'2345'5000ull },   // addr just below base
        };
        bool underflow_exact{ true };
        for (const uf u : cases)
        {
            const std::uint32_t got{ narrow_encode(u.base, u.shift, u.addr) };
            const std::uint32_t want{
                static_cast<std::uint32_t>((u.addr - u.base) >> u.shift) };
            if (got != want) { underflow_exact = false; }
        }
        check("klass_encode_underflow_exact_modular_value", underflow_exact);

        // The named single case kept explicit: (0 - 0x800000000) mod 2^64 is
        // 0xFFFFFFF800000000; >> 3 == 0x1FFFFFFF00000000; narrowed to u32 == 0.
        check("klass_encode_zero_minus_32G_shift3_is_zero",
              narrow_encode(0x8'0000'0000ull, 3u, 0u) == 0u);
        // (0 - 0x800000000) >> 0 == 0xFFFFFFF800000000; low 32 bits == 0.
        check("klass_encode_zero_minus_32G_shift0_low32_is_zero",
              narrow_encode(0x8'0000'0000ull, 0u, 0u) == 0u);
        // base-1 with shift 0: (-1) mod 2^64 == 0xFFFF...FFFF; low32 == 0xFFFFFFFF.
        check("klass_encode_base_minus_1_shift0_is_all_ones",
              narrow_encode(0x8'0000'0000ull, 0u, 0x8'0000'0000ull - 1u) == 0xFFFF'FFFFu);
    }

    // =====================================================================
    // W. EXACT 64-bit decoded VALUE spec for representative (base, shift, narrow)
    //    triples - not just "matches the re-implemented formula" (section C) but a
    //    hand-computed literal, so a regression in the widening/shift/add is caught
    //    even if BOTH the library and the test's expect_decode drifted the same
    //    way.  Each literal is derived by hand from base + (uint64(c) << shift).
    // =====================================================================
    {
        // base 0, shift 0: identity - decoded == narrow, zero-extended.
        check("klass_decode_literal_zero_base_shift0_identity",
              as_uptr(narrow_decode(0u, 0u, 0x1234'5678u)) == 0x1234'5678ull);
        // base 0, shift 3: narrow * 8.
        check("klass_decode_literal_zero_base_shift3_times8",
              as_uptr(narrow_decode(0u, 3u, 0x1000'0000u)) == 0x8000'0000ull);
        // 32G base, shift 3, narrow 1: base + 8.
        check("klass_decode_literal_32G_base_shift3_one",
              as_uptr(narrow_decode(0x8'0000'0000ull, 3u, 1u)) == 0x8'0000'0008ull);
        // 32G base, shift 0, narrow 0xFFFFFFFF: base + 0xFFFFFFFF (no overflow).
        check("klass_decode_literal_32G_base_shift0_allones",
              as_uptr(narrow_decode(0x8'0000'0000ull, 0u, 0xFFFF'FFFFu))
                  == 0x8'FFFF'FFFFull);
        // high CDS-ish base 0x7F0000000000, shift 3, narrow 0x2: base + 16.
        check("klass_decode_literal_cds_base_shift3_two",
              as_uptr(narrow_decode(0x7F00'0000'0000ull, 3u, 2u))
                  == 0x7F00'0000'0010ull);
        // sign-boundary narrow 0x80000000 at shift 3 from base 0: exactly 2^34.
        check("klass_decode_literal_signbit_shift3_is_2pow34",
              as_uptr(narrow_decode(0u, 3u, 0x8000'0000u)) == (std::uint64_t{ 1u } << 34));
        // 0x7FFFFFFF (max positive int32) at shift 3: (2^31 - 1) * 8.
        check("klass_decode_literal_int32max_shift3",
              as_uptr(narrow_decode(0u, 3u, 0x7FFF'FFFFu))
                  == ((std::uint64_t{ 0x7FFF'FFFFull }) << 3));
    }

    // =====================================================================
    // X. EXACT 32-bit encoded VALUE spec - encode literals, the inverse of W.
    //    narrow_encode(base, shift, addr) == uint32((addr - base) >> shift); each
    //    expected value hand-derived.  Covers the on-grid (lossless) and a couple
    //    of off-grid (floored) addresses to pin the >> truncation precisely.
    // =====================================================================
    {
        // On-grid: addr = base + (c << shift) round-trips to c exactly.
        check("klass_encode_literal_ongrid_shift3",
              narrow_encode(0x8'0000'0000ull, 3u, 0x8'0000'0000ull + (0x1234ull << 3))
                  == 0x1234u);
        check("klass_encode_literal_ongrid_shift0",
              narrow_encode(0x8'0000'0000ull, 0u, 0x8'0000'0000ull + 0xABCDu) == 0xABCDu);
        // Off-grid floor: base + 7 at shift 3 -> 7 >> 3 == 0 (floors to base).
        check("klass_encode_literal_offgrid_7_shift3_floors_to_zero",
              narrow_encode(0x8'0000'0000ull, 3u, 0x8'0000'0000ull + 7u) == 0u);
        // Off-grid floor: base + 15 at shift 3 -> 15 >> 3 == 1.
        check("klass_encode_literal_offgrid_15_shift3_floors_to_one",
              narrow_encode(0x8'0000'0000ull, 3u, 0x8'0000'0000ull + 15u) == 1u);
        // Off-grid floor: base + 0xFFFF at shift 4 -> 0xFFFF >> 4 == 0xFFF.
        check("klass_encode_literal_offgrid_shift4_floor",
              narrow_encode(0x8'0000'0000ull, 4u, 0x8'0000'0000ull + 0xFFFFu) == 0xFFFu);
        // Top-of-range on-grid: base + (0xFFFFFFFF << 3) encodes to 0xFFFFFFFF.
        check("klass_encode_literal_top_shift3",
              narrow_encode(0x8'0000'0000ull, 3u,
                            0x8'0000'0000ull + (std::uint64_t{ 0xFFFF'FFFFull } << 3))
                  == 0xFFFF'FFFFu);
    }

    // =====================================================================
    // Y. ENCODE TRUNCATION beyond 32 bits - narrow_encode narrows the shifted
    //    difference to uint32, discarding any bits at/above bit 32.  If a decoded
    //    address is so far above base that (addr-base)>>shift exceeds 0xFFFFFFFF,
    //    encode keeps ONLY the low 32 bits.  This is a documented lossy edge (an
    //    address outside the representable narrow window).  Pin the exact low-32
    //    truncation for several over-range differences.
    // =====================================================================
    {
        // diff = 0x1'0000'0000 (2^32) at shift 0 -> low32 == 0.
        check("klass_encode_diff_2pow32_shift0_truncates_to_zero",
              narrow_encode(0u, 0u, std::uint64_t{ 0x1'0000'0000ull }) == 0u);
        // diff = 0x1'0000'0001 at shift 0 -> low32 == 1.
        check("klass_encode_diff_2pow32_plus1_shift0_low32_is_one",
              narrow_encode(0u, 0u, std::uint64_t{ 0x1'0000'0001ull }) == 1u);
        // diff = 0x7'FFFF'FFF8 at shift 3 -> >>3 == 0xFFFFFFFF (fits, the max).
        check("klass_encode_max_window_shift3_is_all_ones",
              narrow_encode(0u, 3u, std::uint64_t{ 0x7'FFFF'FFF8ull }) == 0xFFFF'FFFFu);
        // diff = 0x8'0000'0000 at shift 3 -> >>3 == 0x1'0000'0000 -> low32 == 0.
        check("klass_encode_just_past_window_shift3_truncates_to_zero",
              narrow_encode(0u, 3u, std::uint64_t{ 0x8'0000'0000ull }) == 0u);
        // diff = 0x8'0000'0008 at shift 3 -> >>3 == 0x1'0000'0001 -> low32 == 1.
        check("klass_encode_just_past_window_shift3_low32_is_one",
              narrow_encode(0u, 3u, std::uint64_t{ 0x8'0000'0008ull }) == 1u);
    }

    // =====================================================================
    // Z. resolve_struct_entry - EXTRA structural coverage beyond section N.
    //    Re-pin the walker's first-match semantics with a controlled list whose
    //    LAST element is the only one that could ever match (still nullptr off-JVM
    //    since none exist), a list where a real-looking klass name sits AFTER a
    //    bogus one (order independence proven by all-miss), and a maximal-length
    //    bogus list to exercise the full loop bound.  All deterministic: nullptr.
    // =====================================================================
    {
        using vmhook::hotspot::resolve_struct_entry;
        using vmhook::hotspot::struct_entry_candidate_t;

        // (Z1) A list whose ONLY plausibly-real entry is last: off-JVM the walker
        //      visits every earlier bogus entry, then the real-name one, and STILL
        //      returns nullptr because gHotSpotVMStructs is absent.
        static constexpr struct_entry_candidate_t real_last[]{
            { "Bogus1", "_x" }, { "Bogus2", "_y" },
            { "CompressedKlassPointers", "_narrow_klass._base" },
        };
        check("resolve_real_name_last_no_jvm_is_null",
              resolve_struct_entry(real_last, std::size(real_last)) == nullptr);

        // (Z2) A list whose FIRST entry is a real klass name: first-match-wins
        //      would return it IF it resolved; off-JVM it does not, so nullptr.
        static constexpr struct_entry_candidate_t real_first[]{
            { "CompressedKlassPointers", "_base" },
            { "Bogus", "_z" },
        };
        check("resolve_real_name_first_no_jvm_is_null",
              resolve_struct_entry(real_first, std::size(real_first)) == nullptr);

        // (Z3) Empty field / empty type strings are passed through to
        //      iterate_struct_entries verbatim and miss (no entry has an empty
        //      name) -> nullptr, no crash.
        static constexpr struct_entry_candidate_t empty_names[]{
            { "", "" }, { "CompressedKlassPointers", "" }, { "", "_base" },
        };
        check("resolve_empty_name_candidates_no_jvm_is_null",
              resolve_struct_entry(empty_names, std::size(empty_names)) == nullptr);

        // (Z4) A long bogus list (32 entries) walks the entire loop and returns
        //      nullptr - pins that count is honoured for large counts and there is
        //      no early false-positive anywhere in the scan.
        static constexpr struct_entry_candidate_t long_bogus[]{
            { "T00", "_f" }, { "T01", "_f" }, { "T02", "_f" }, { "T03", "_f" },
            { "T04", "_f" }, { "T05", "_f" }, { "T06", "_f" }, { "T07", "_f" },
            { "T08", "_f" }, { "T09", "_f" }, { "T10", "_f" }, { "T11", "_f" },
            { "T12", "_f" }, { "T13", "_f" }, { "T14", "_f" }, { "T15", "_f" },
            { "T16", "_f" }, { "T17", "_f" }, { "T18", "_f" }, { "T19", "_f" },
            { "T20", "_f" }, { "T21", "_f" }, { "T22", "_f" }, { "T23", "_f" },
            { "T24", "_f" }, { "T25", "_f" }, { "T26", "_f" }, { "T27", "_f" },
            { "T28", "_f" }, { "T29", "_f" }, { "T30", "_f" }, { "T31", "_f" },
        };
        check("resolve_long_bogus_list_no_jvm_is_null",
              resolve_struct_entry(long_bogus, std::size(long_bogus)) == nullptr);

        // (Z5) The klass and oop candidate type names are DIFFERENT strings, the
        //      compile-anchored proof the two codecs target different VMStructs.
        //      decode_klass_pointer uses "CompressedKlassPointers"; decode_oop_
        //      pointer uses "CompressedOops". A single literal strcmp pins it.
        check("klass_vs_oop_codec_type_names_differ",
              std::strcmp("CompressedKlassPointers", "CompressedOops") != 0);
    }

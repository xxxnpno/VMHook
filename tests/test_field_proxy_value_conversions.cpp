...
int main()
{
    ...
    //     must NEVER fault on a bad pointer (it falls back to the
    //     kernel-validated os::safe_read when its cheap probe trips).  These
    //     cases run with no JVM — they exercise the read primitive directly
    //     over buffers we own plus the same 0xDEAD0000 unmapped sentinel
    //     section 34 uses.
    // ---------------------------------------------------------------------
    {
        ...
        // Cheap-path FAULT recovery: an unmapped source must NOT fault — the
        // cheap probe trips, falls back to safe_read, and reports false (same
        // as safe_read).  This is the load-bearing fault-safety case: if the
        // SEH / signal guard failed to catch, this would crash the process.
        {
        ...
        // valid read must all behave (no wedged signal handler / SEH filter, no
        // fault).  Closest no-JVM proxy that the install-once handler stays
        // healthy across repeated cheap-path trips.
        {
            void* const unmapped{ reinterpret_cast<void*>(
                static_cast<std::uintptr_t>(0xDEAD0000ULL)) };
            bool all_false{ true };

...
//
// ---------------------------------------------------------------------------
// JIT-RELIABILITY HARDENING (why this module deopts trigger() before driving)
// ---------------------------------------------------------------------------
// The whole battery is reached through an i2i INTERPRETER detour on
// CollList.trigger(int): the detour fires (and does ALL the to_vector reads on
// `self`) only when trigger() is dispatched through the INTERPRETER, so the
// patched i2i stub is reached.  A JIT-compiled (i2c/nmethod) dispatch BYPASSES
// the i2i patch and the detour never fires -> g_detour_calls stays 0, g_self_ok
// stays false, every list stays unobserved, and EVERY arraylist_*/linkedlist_*/
// object_array_*/nested/generic_fallback check FAILs deterministically (plus the
// probe-completed/detour-fired/saw-self infra checks).
//
// This is exactly the failure that quarantined the module on the new MSVC 14.51
// windows runner (+ windows·clang·java25): the suite grew ~40% (waves 1-3) and
// the new runner JITs aggressively, so trigger() can already be JIT-warm at
// install time (or get asynchronously recompiled in the microsecond window
// between install and the asserting drive) -> the capture never happens.  It is
// the SAME class of flake hook_basic.cpp just fixed for its hooked methods.
//
// The proven fix (mirrored verbatim from hook_basic.cpp, which is green) is to
// DEOPTIMIZE trigger() after install so execution returns to the interpreter and
// the i2i detour is taken.  We do it with a BOUNDED settle loop
// (settle_interpreter_route): fixture-scoped deoptimize_methods_if(CollList) +
// verify_hooks() until the live Method's interpreter entry is observed routing
// through the i2i stub (_from_interpreted_entry == _i2i_entry, _code == null) on
// a 40 ms cadence.  vmhook holds NO_COMPILE on the hooked Method (set at
// install), so once the route is established it stays put for the single Java
// probe.  drive_capture_until_fires() re-settles + re-drives the probe up to a
// small budget so a recompile racing between the settle and the START of run()
// cannot slip the call past the detour.  EVERY arraylist_*/linkedlist_*/
// object_array_* content check stays HARD on the final observation — the
// settle/retry only EXISTS to make the capture deterministic; a genuine failure
// to capture after the whole budget still FAILs the HARD asserts (a real
// regression).
...
namespace
{
    ...
    public:
        ...
        // ── handshake ──────────────────────────────────────────────────────
        // set_done(false) clears the LATCHED done flag on the rising edge so the
        // fixture's pending() (go && !done) re-arms and run() re-fires — required
        // by drive_capture_until_fires's bounded retry (the fixture never clears
        // done itself, so a stale latch would block every re-drive).
...
namespace
{
    ...
    // Used to locate the live trigger() Method* (for the interpreter-route
    // settle), as the entry-guard lookup key, AND as the predicate filter for
    // deoptimize_methods_if so the deopt is scoped to THIS fixture's methods only
    // (so settling trigger() never disturbs another module's hot methods).
    constexpr char FIXTURE[]{ "vmhook/fixtures/CollList" };

    // ── JIT-reliability helpers (mirrored from hook_basic.cpp) ───────────────
    // The capture detour fires only on an INTERPRETED dispatch of trigger(); a
    // JIT-compiled dispatch bypasses the i2i patch.  These drive trigger() back
    // to the interpreter route before the asserting probe so the capture is
    // deterministic on the fast-JIT runners (MSVC 14.51 / clang·java25) that
    // quarantined this module.  See the JIT-RELIABILITY HARDENING banner at top.

    // Locates the live Method* for FIXTURE::name(signature) by walking the
    // InstanceKlass methods array.  Returns nullptr if anything looks invalid —
    // callers treat nullptr as "cannot settle the interpreter route" and fall
    // back to driving without the settle (never a crash).  All reads are
    // pointer-validated.  (Same shape as hook_basic.cpp's find_method.)
    auto find_method(const char* const name, const char* const signature)
        -> vmhook::hotspot::method*
    {
        vmhook::hotspot::klass* const k{ vmhook::find_class(FIXTURE) };
    ...
    // path and verify_hooks()/deoptimize_methods_if re-establish.  Once HotSpot
    // re-JITs the method, _from_interpreted_entry is repointed at the i2c adapter
    // and this returns false.  Pointer-validated; unreadable entries yield false.
    // (Mirrors hook_basic.cpp's interp_routes_through_i2i.)
    ...
    // Drives the hooked trigger() back to the interpreter so the next dispatch
    // reaches the i2i patch, with a bounded tolerance for HotSpot's ASYNCHRONOUS
    // tiered JIT (which can compile/recompile at any instant, including the window
    // right after install).  Returns true once the interpreter route is observed
    // established, or when there is no live Method* to settle (m == nullptr — the
    // caller then just drives, the pre-hardening behaviour, still correct on a
    // cold/interpreted method).
    //
    // Each attempt: (1) deoptimize_methods_if(<our fixture class>) — for any
    // CURRENTLY-compiled fixture method this repoints _from_interpreted_entry ->
    // i2i, _from_compiled_entry -> c2i and nulls _code, i.e. the exact deopt the
    // install path performs for an already-JIT'd method; (2) verify_hooks() —
    // re-arms NO_COMPILE / re-applies the hook's deopt (and re-points the
    // interpreter entry when _code != null), absorbing an in-flight recompile that
    // just landed; (3) re-check the route, with a short settle so a queued nmethod
    // lands + is re-nulled before the next read.
    //
    // Non-vacuous: a transient async recompile is ABSORBED (a later attempt sees
    // the route restored), but if trigger() genuinely cannot be driven to the
    // interpreter route within the budget this returns false and the caller's
    // drive_capture_until_fires falls through to its HARD checks (a real
    // regression then fails the suite — the content checks are NOT softened).
    auto settle_interpreter_route(vmhook::hotspot::method* const m, int attempts) -> bool
    {
        ...
        for (int attempt{ 0 }; attempt < attempts; ++attempt)
        {
            ...
            // 40 ms matches the proven hook_basic.cpp cadence for the same
            // fast-JIT async-recompile race.
            std::this_thread::sleep_for(std::chrono::milliseconds{ 40 });
        }
        return interp_routes_through_i2i(m) && method_code(m) == nullptr;
    }

    // Drives the go/done capture probe and guarantees the trigger() detour fires
    // (the capture happens) by re-settling the interpreter route and re-driving
    // up to `attempts` times.  `done_out` receives the probe-completed status of
    // the FINAL drive (an infra signal the caller asserts hard regardless).  The
    // detour latches CollList.done only after it runs, so a fire is observable
    // BOTH via `done` and via g_detour_calls; we retry until g_detour_calls > 0
    // (the capture actually executed) so the heavy to_vector battery has run.
    //
    // Because CollList.done LATCHES across the suite (the fixture never clears it)
    // we must FORCE the rising edge: clear done before raising go each attempt, so
    // pending() (go && !done) re-arms and the fixture's run() re-executes.  This
    // wrapper only EXISTS to make the capture deterministic; the caller's content
    // checks stay HARD on the final observation.
    auto drive_capture_until_fires(vmhook_test::context& ctx,
                                   ...
                                   bool& done_out) -> void
    {
        ...
        for (int attempt{ 0 }; attempt < attempts; ++attempt)
        {
            ...
            done_out = ctx.run_probe(
                [](bool value)
                {
                    if (value)
                    {
                        // Rising edge: clear the latched `done` BEFORE raising go
                        // so pending() (go && !done) re-arms and run() re-fires.
                        coll_list_fixture::set_done(false);
                    }
                    coll_list_fixture::set_go(value);
                },
                []() { return coll_list_fixture::get_done(); });
            if (done_out && g_detour_calls.load(std::memory_order_relaxed) >= 1)
            {
                return;   // capture executed deterministically
            }
...

    {
        ...
        // Deopt trigger() back to the interpreter so the i2i capture detour is
        // taken even if HotSpot JIT-compiled it before install (the fast-JIT
        // MSVC-14.51 / clang·java25 race that quarantined this module), then drive
        // the go/done probe, re-settling + re-driving until the capture executes.
        // trigger(int)->int has JVM signature "(I)I".  find_method returns nullptr
        // if the live Method* can't be located (then we just drive — still correct
        // on a cold/interpreted method); the content checks below stay HARD on the
        // final observation regardless.
        vmhook::hotspot::method* const trigger_m{ find_method("trigger", "(I)I") };
        bool done{ false };
        drive_capture_until_fires(ctx, trigger_m, 6, done);

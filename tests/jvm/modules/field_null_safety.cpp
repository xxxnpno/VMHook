...
namespace
{
    ...
    // Fixture `mode` selector values (lockstep with FieldNullSafety.java).
    constexpr std::int32_t kModeMutateOkInt{ 1 };  // putstatic okInt (live-state proof)
    constexpr std::int32_t kModeGcSettle{ 2 };     // tenure this class's mirror
}
...
VMHOOK_JVM_MODULE(field_null_safety)
{
    ...
    // =====================================================================
    //  PRELUDE — TENURE THIS CLASS'S java.lang.Class MIRROR before any static
    //  VALUE read.  WHY (CI-only, windows·msvc 14.51): the library's static
    //  read path resolves `mirror + offset` and then does a RAW (GC-unsafe)
    //  read of that address.  The mirror is a young, relocatable heap oop the
    //  first time this module touches it; a young GC firing between resolve and
    //  read leaves the captured address pointing at the pre-relocation copy —
    //  stale bytes (a WRONG value -> a clean [FAIL]) or, if the old page is
    //  unmapped, a FAULT (-> the JVM dies, "no TOTAL").  This is the same
    //  accepted raw-oop GC-safety gap deflaked in collection_list; it was
    //  invisible on the old runner but the msvc-14.51 codegen/timing widened
    //  the window enough to corrupt the FIRST handful of reads (~3/4 runs) on
    //  java 24/25/26, while windows·clang and every linux/macos JDK stayed
    //  green.  Driving the fixture's gcSettle() (garbage + System.gc() x2)
    //  promotes the mirror to old-gen, where G1 will not relocate it, so every
    //  value read below is stable and can stay a HARD ctx.check.
    //
    //  This is best-effort warm-up, NOT an assertion: it is recorded as [INFO]
    //  and can never FAIL the suite.  If it does not complete (e.g. the probe
    //  pump is busy), the value checks below would surface a genuine problem on
    //  their own — we never weaken them.
    // =====================================================================
    {
        const bool tenured{ ctx.run_probe(
            [](bool value)
            {
                if (value)
                {
                    fns::set_done(false);
                    fns::set_mode(kModeGcSettle);
                }
                fns::set_go(value);
            },
            []() { return fns::get_done(); }) };
        ctx.record(std::string{ "[INFO] field_null_safety: pre-read mirror tenure "
                                "(gcSettle: garbage + System.gc() x2) completed=" }
                   + (tenured ? "yes" : "no")
                   + " — promotes this class's java.lang.Class mirror to old-gen so the "
                     "library's GC-unsafe raw static reads cannot race a relocating young "
                     "GC (windows·msvc-14.51 first-touch fix).");
    }
    ...
    // =====================================================================
    {
        // Rising-edge handshake: select mode 1 (mutateOkInt's putstatic) and
        // clear the latched `done` BEFORE raising go.  Resetting mode is
        // required because the PRELUDE above ran mode 2 (gcSettle) and left
        // done==true; without this the probe would re-run gcSettle (or observe
        // a stale done) instead of performing the putstatic this phase proves.

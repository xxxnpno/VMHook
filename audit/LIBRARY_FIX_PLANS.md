...
- **Fix (test-side, applied):** guard the crash-prone reads with `#if (defined(_MSC_VER) && !defined(__clang__)) || !defined(_WIN32)` (real MSVC SEH + POSIX only; skip mingw/clang-cl), mirroring dont_inline scenario 7. **Fix (library):** route every DECODED-oop header/element read through os::safe_read so a cold read returns the existing fallback instead of faulting; method-dispatch-on-cold-element stays test-gated.
- **Re-scoped by the 2026-06-21 design pass (decode_oop_and_pointers-specialist):** the compressed-VALUE slot read (`(a)`) is ALREADY hardened everywhere (field_proxy::get/get_compressed_oop via safe_read_fast, read_java_string/symbol::to_string/extract_frame_arg/klass_from_oop via safe_read). The residual fault is reading the DECODED oop's header (+8 klass, +12 length) and element (+16) — `(b)`/`(c)`. JNI dispatch on a cold oop `(d)` is NOT safe_read-fixable, stays test-#if-gated.
  - **TIER 1 — DONE (14c3543).** array_length (+12 length, cross-platform os::safe_read, fallback 0); get_array_element<T> (+16 element, os::safe_read_fast for the hot loop, fallback T{}); klass_from_object_header (+8 narrow-klass, Windows-gated safe_read twin of klass_from_oop, fallback nullptr). Additive/warm-path byte-identical. Reaches the biggest crash surface (field_object_ref, field_arrays_object, collection table-head, method_overload).
  - **TIER 2 — DONE (e0a7591).** New `read_compressed_oop_at_safe(holder,offset)` helper (fault-safe twin of read_compressed_oop_at via os::safe_read_fast, fallback 0); 17 raw node-slot reads converted across all 4 walkers — hash_map_walk_entries (table/key/value/next), hash_map_walk_keys (table/key/next), tree_map_walk_entries (root/left/key/value/right), tree_map_walk_keys (root/left/key/right). Additive/byte-identical; a cold node → 0 → nullptr → clean walk termination. Forced -Werror exit 0.
  - **TIER 3 — DEFER/low-pri.** S4 klass_match_ok narrow-klass memcpy (~15131, fold into Tier 2 if touching that region, fail-open=true); S7 write_java_string value-slot READ (~20612, READ only — do NOT touch the writes per the #4 revert); D1 the getTag() cold-dispatch (field_arrays_object.cpp:131/150) stays permanently test-#if-gated.
- **Validation:** flake is GC-timing intermittent + locations MOVE, so a single green is not proof — watch the no-SEH cells (windows·{mingw,clang}·8-26) across SEVERAL runs after each tier; crash-rate should drop. Tier 1 was low-risk (additive, sibling-proven #2/#13 pattern) → landed direct to master, not trial-branched.
- **Risk:** the #if guard removes the read coverage on mingw/clang-cl (kept on MSVC + linux + macos). The library os::safe_read tiers are the real cure for the decode-and-read half.
